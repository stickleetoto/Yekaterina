...
- **1,429** built-in/control operations in the final target manifest.
...
`yk.find` now recognizes more common intent phrases without adding a new MCP tool or duplicating operations that already exist.
...
See `docs/V14_FREE_COMPATIBILITY_CONTRACT.md` for the complete contract.

## Verification requirements

The exact final source is promotable only after all release gates pass:
...
v1.4.0 is intended as the final public feature-development baseline. The v1.4.x line is maintenance-oriented and may receive correctness, security, compatibility, packaging and discovery-only fixes.

The next feature line is developed separately from this public baseline.

## Release-candidate note

These notes are prepared during finalization. The release is not considered published until package metadata is promoted to `1.4.0`, the exact candidate passes every gate above, the Windows x64 package and checksum are produced, and the packaged binary passes its smoke test.
